    ...
    <div class="row">
        <div class="col-12">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white border-bottom py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="text-muted mb-0">Add a new product category to your catalog</p>
                        </div>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.catalog-categories.store') }}" class="needs-validation" novalidate>
                        @csrf
                                    ...
                                    <select name="interest_id" class="form-select @error('interest_id') is-invalid @enderror" id="interest_id">
                                        ...
                                        @foreach($interests as $interest)
                                            <option value="{{ $interest->id }}"
                                                {{ old('interest_id') == $interest->id ? 'selected' : '' }}>
                                                {{ $interest->name }}
                                            </option>
                                        @endforeach
                                    </select>
    ...
    if (iconInput && iconPreview) {
        iconInput.addEventListener('input', function() {
            if (this.value) {
                ...
                iconPreview.innerHTML = '<i class="fas fa-folder fa-2x text-primary"></i>';
            }
        });
    }
